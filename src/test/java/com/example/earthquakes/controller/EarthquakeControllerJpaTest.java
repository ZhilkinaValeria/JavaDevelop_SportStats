...
import com.example.earthquakes.config.TestJpaConfig;
...
class EarthquakeControllerJpaTest {
    ...
    void create_WithAdminRole_ShouldCreateEarthquake() throws Exception {
        when(earthquakesJpaService.create(any(Earthquake.class))).thenReturn(newEarthquake);
        ...
        verify(earthquakesJpaService, times(1)).create(any(Earthquake.class));
    }
    ...
    void create_WithUserRole_ShouldReturnForbidden() throws Exception {
        ...
        verify(earthquakesJpaService, never()).create(any());
    }
    ...
    void getById_WithUserRole_ShouldReturnEarthquake() throws Exception {
        ...
        when(earthquakesJpaService.getById("1")).thenReturn(earthquake);
    ...
    void getAll_ShouldReturnList() throws Exception {
        ...
        when(earthquakesJpaService.getAll()).thenReturn(earthquakes);
        ...
        verify(earthquakesJpaService, times(1)).getAll();
    }
    
    @Test
    @WithMockUser(roles = "ADMIN")
    void delete_WithAdminRole_ShouldDelete() throws Exception {
        doNothing().when(earthquakesJpaService).delete("1");
        
        mockMvc.perform(delete("/api/earthquakes/1")
                .with(csrf()))
            .andExpect(status().isNoContent());
        
        verify(earthquakesJpaService, times(1)).delete("1");
    }
